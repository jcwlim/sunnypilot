...
def create_acc_commands(packer, enabled, accel, upper_jerk, idx, hud_control, set_speed, stopping, long_override, use_fca, CP,
  ...
  def get_scc12_values():
    scc12_values = {
      ...
      "StopReq": 1 if stopping else 0, #1 if tuning.stopping else 0,
      "aReqRaw": 0 if stopping else 0 if accel <= 0.08 and accel >= -0.08 else accel, #tuning.desired_accel,
      "aReqValue": accel, #tuning.actual_accel,  # stock ramps up and down respecting jerk limit until it reaches aReqRaw
  ...
  def get_scc14_values():
    return {
      "ComfortBandUpper": 0, #tuning.comfort_band_upper, # stock usually is 0 but sometimes uses higher values
      "ComfortBandLower": 0, #tuning.comfort_band_lower, # stock usually is 0 but sometimes uses higher values
      "JerkUpperLimit": 0.0 if accel <= 1 else 1.0, #tuning.jerk_upper, # stock usually is 1.0 but sometimes uses higher values
      "JerkLowerLimit": 0.0 if accel >= 0 else 5.0, #tuning.jerk_lower, # stock usually is 0.5 but sometimes uses higher values
