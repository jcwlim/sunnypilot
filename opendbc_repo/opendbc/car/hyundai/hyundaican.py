...
def create_acc_commands(packer, enabled, accel, upper_jerk, idx, hud_control, set_speed, stopping, long_override, use_fca, CP,
  ...
  def get_scc11_values():
    return {
      ...
      "ObjValid": 1 if hud_control.leadVisible else 0, # close lead makes controls tighterAdd commentMore actions
      "ACC_ObjStatus": 1 if hud_control.leadVisible else 0, # close lead makes controls
      "ACC_ObjLatPos": 0,
      "ACC_ObjRelSpd": 0,
  ...
  def get_scc12_values():
    scc12_values = {
      ...
      "aReqRaw": 0 if stopping else 0 if accel <= 0.08 and accel >= -0.08 else accel, #tuning.desired_accel,Add commentMore actions
  ...
  def get_scc14_values():
    return {
      ...
      "JerkUpperLimit": 0.0 if accel <= 1 else 1.0, #tuning.jerk_upper, # stock usually is 1.0 but sometimes uses higher valuesAdd commentMore actions
